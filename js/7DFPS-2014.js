...
// The piece in front of the camera, and its HUD twin drawn on top of the world
let heldMesh;
const meshHUD = new THREE.Mesh(triHexGeometry, triHexMaterial);

// Places triHex mesh in front of camera
function makeTriHexMesh() {
  heldMesh = new THREE.Mesh(triHexGeometry, triHexMaterial);
...
function updateTriHexPositions() {
  ...
  for (let i = 0; i < triHexMeshes.length - 1; i += 1) {
...
function updateHUD() {
  ...
  heldMesh.matrixWorld.decompose(
    meshHUD.position,
    meshHUD.quaternion,
    meshHUD.scale
  );
...
function shootTriHexMesh() {
  // Detach piece from camera; detach leaves its position in world coordinates
  THREE.SceneUtils.detach(heldMesh, camera, scene);
  // Remember to delete the object later

  // Move back up to center of the screen
  heldMesh.position.y += 10;
...
function rotateTriHexMesh(degrees) {
  // Clockwise
  heldMesh.rotateZ(-THREE.Math.degToRad(degrees));
}
